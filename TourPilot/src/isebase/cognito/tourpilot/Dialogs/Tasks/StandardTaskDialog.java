...
//				if ((editable.toString().contains(",") || editable.toString().endsWith(".")) || (editable.toString().split(",").length > 1))
//				{
//					editable.delete(editable.length() - 1, editable.length());
//					editable.append(",");
//				}
				Matcher matcher = pattern.matcher(editable.toString().charAt(editable.length() - 1) + "");
				if(matcher.matches()) {
					etValue.addTextChangedListener(this);
					return;
				}
				char c = editable.toString().charAt(editable.length() - 1);
				editable.delete(editable.length() - 1, editable.length());
				if(!editable.toString().contains(",") || editable.toString().charAt(editable.length() - 1) == '.' )
					editable.append(',');
				etValue.addTextChangedListener(this);
			}
