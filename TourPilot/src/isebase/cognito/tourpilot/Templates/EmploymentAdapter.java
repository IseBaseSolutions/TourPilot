...
public class EmploymentAdapter extends ArrayAdapter<Employment> {
	...
	public EmploymentAdapter(Context context, int layoutResourceId,
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		employmentHolder.tvEmploymentName = (TextView) row
				.findViewById(R.id.tvEmploymentName);
		employmentHolder.tvEmploymentName.setText(employment.getName());

		employmentHolder.tvEmploymentTime = (TextView) row
				.findViewById(R.id.tvEmploymentTime);
		employmentHolder.tvEmploymentTime.setText(employment.getTime());

		if (employment.isDone()) {
			employmentHolder.tvEmploymentName.setTextColor(StaticResources
					.getBaseContext().getResources()
					.getColor(R.color.activeColor));
			employmentHolder.tvEmploymentTime.setTextColor(StaticResources
					.getBaseContext().getResources()
					.getColor(R.color.activeColor));
		}
		if (employment.isAborted()) {
			employmentHolder.tvEmploymentName.setTextColor(StaticResources
					.getBaseContext().getResources()
					.getColor(R.color.notActveColor));
			employmentHolder.tvEmploymentTime.setTextColor(StaticResources
					.getBaseContext().getResources()
					.getColor(R.color.notActveColor));
		}
